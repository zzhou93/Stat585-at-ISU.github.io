...
##    weather temperature_string             wind_string
## 1 Overcast    54.0 F (12.2 C) South at 9.2 MPH (8 KT)
```
...
I investigated  `<html>`, `<p>`, `<h1>`, `<body>`, `<style>`, `<image>`, etc.

We can use an internal CSS to format separate piece of a document. An internal CSS is defined in the `<head>` section of an HTML page, within a `<style>` element. For example if we want to format the document background, the headings and the paragraph. Then we can use:
...
Answer the following questions:
    ...
    I found tags like: `<head>`, `<body>`, `<p>`, `<style>`, `<div>`, `<script>`, `<code>`, etc.
    - How are code chunks formatted in HTML?
    
    The format is `<pre class="r">`
    - What differences are there in the HTML markup for R code chunks and R output blocks?
    
    The output block doesn't follow `<pre class="r">`, and it doesn't have the tag `<span>` which I think it's used for codes formatting.
...
![center](./../figure/09/HuDapeng/unnamed-chunk-3-1.png)
---
>>>>>>> blog 9 posts up
